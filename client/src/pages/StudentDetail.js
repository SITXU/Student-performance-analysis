...
export default function StudentDetail() {
  ...
  useEffect(() => {
    api.get(`/analytics/student/${reg}`)
      .then(r => setData(r.data))
      .catch(() => navigate(-1))
      .finally(() => setLoading(false));
  }, [reg]);

  if (loading) return <div className="loading">Loading student report...</div>;
  if (!data) return null;
    ...
    datasets: [
      {
        ...
        pointBackgroundColor: '#00d4ff', borderWidth: 2, pointRadius: 5,
      },
      {
        label: 'Class Avg',
        data: Object.values(classCoAvg),
        borderColor: '#7c3aed', backgroundColor: 'rgba(124,58,237,0.05)',
        borderDash: [5, 5], pointBackgroundColor: '#7c3aed', borderWidth: 1.5, pointRadius: 3,
      }
    ]
  };
  ...
  return (
    <div className="page">
      ...
      <div className="student-header card">
        ...
        <div className="student-info">
          ...
          <div className="student-meta">
            <div className="meta-item"><span>Reg. No.</span><code>{s.reg}</code></div>
            <div className="meta-item"><span>Total Score</span><strong>{s.total}/30</strong></div>
            <div className="meta-item"><span>Percentage</span><strong>{pct}%</strong></div>
            <div className="meta-item">
              <span>Status</span>
              <span className={`pill ${pct >= 60 ? 'pill-green' : 'pill-red'}`}>{pct >= 60 ? 'PASS' : 'AT RISK'}</span>
            </div>
            {rank && <div className="meta-item"><span>Class Rank</span><strong>#{rank} of {totalStudents}</strong></div>}
          </div>
        </div>
      </div>
      ...
      <div className="charts-row">
        <div className="card">
          <div className="card-title">CO Attainment vs Class Average</div>
      ...
      <div className="card">
        ...
        <div className="co-summary-grid">
          {Object.entries(coScores).map(([co, pct]) => (
            <div key={co} className="co-summary-card">
              <div className="co-summary-name" style={{ color: CO_COLORS[co] }}>{co}</div>
              <div className="co-summary-pct" style={{ color: CO_COLORS[co] }}>{pct}%</div>
              <div className="progress-bar">
                <div className="progress-fill" style={{ width: `${pct}%`, background: CO_COLORS[co] }} />
              </div>
              <div className={`pill ${pct >= 50 ? 'pill-green' : 'pill-red'}`} style={{ marginTop: 8, fontSize: 11 }}>
                {pct >= 50 ? 'Achieved' : 'Not Achieved'}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="card">
          ...
          <div className="q-chips">
            {Object.entries(CO_MAP).map(([q, co]) => {
              const mark = (s.q || {})[q] ?? 0;
              const max = Q_MAX[q];
              const mpct = Math.round((mark / max) * 100);
              const bg = mpct >= 75 ? 'rgba(16,185,129,0.15)' : mpct >= 50 ? 'rgba(245,158,11,0.15)' : 'rgba(239,68,68,0.15)';
              const color = mpct >= 75 ? '#10b981' : mpct >= 50 ? '#f59e0b' : '#ef4444';
              return (
                <div key={q} className="q-chip" style={{ background: bg, border: `1px solid ${color}33` }}>
                  ...
                  <div className="q-chip-co"><span className={`co-badge co${co[2]}`}>{co}</span></div>
                </div>
              );
            })}
          </div>
